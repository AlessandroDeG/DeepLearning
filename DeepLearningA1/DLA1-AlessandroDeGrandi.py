...
X_train=torch.from_numpy(X_train.reshape((int(sample_size/divide),4))).float().to(DEVICE)
...
plt.plot(np.linspace(x_range[0],x_range[1],1000),[p(x,model.weight.detach().numpy()[0]) for x in np.linspace(x_range[0],x_range[1],1000)] , c='red')
